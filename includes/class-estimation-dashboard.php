...
class Estitofo_Dashboard {
                    ...
                    ?>
                        <li style="padding:6px 0;border-bottom:1px solid #f0f0f1;display:flex;justify-content:space-between;align-items:center;">
                            ...
                            <span style="color:#646970;font-size:12px;">
                                ...
                                · <?php echo esc_html(human_time_diff(strtotime($row->created_at), current_time('timestamp'))); ?>
                            </span>
                        </li>
                    <?php endforeach; ?>
                </ul>
    ...
    public static function statuses() {
        return apply_filters('estitofo_workflow_statuses', array(
    ...
    private static function top_products($days = 30, $limit = 5) {
        global $wpdb;
        $table  = $wpdb->prefix . 'estimation_submissions';
        $cutoff = gmdate('Y-m-d H:i:s', strtotime('-' . absint($days) . ' days', current_time('timestamp')));
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
        ...
        foreach ($rows as $json) {
            ...
            foreach ($items as $item) {
                ...
                if (!isset($counts[$title])) $counts[$title] = 0;
                $counts[$title] += $qty;
            }
        }
        arsort($counts);
