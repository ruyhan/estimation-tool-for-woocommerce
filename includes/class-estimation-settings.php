...
class Estitofo_Settings {
	...
	public static function sanitize_input( array $input ) {
		...
		if ( isset( $input['admin_recipients'] ) ) {
			$clean['admin_recipients'] = self::sanitize_emails_csv( $input['admin_recipients'] );
		}
		if ( isset( $input['from_email'] ) ) {
			$clean['from_email'] = sanitize_email( $input['from_email'] );
		}
		if ( isset( $input['email_body'] ) ) {
			$clean['email_body'] = wp_kses_post( $input['email_body'] );
		}
		if ( isset( $input['locations'] ) ) {
			$clean['locations'] = self::sanitize_locations( $input['locations'] );
		}

		// Colors.
		if ( isset( $input['primary_color'] ) ) {
			$clean['primary_color'] = self::sanitize_color( $input['primary_color'] );
		}
		if ( isset( $input['accent_color'] ) ) {
			$clean['accent_color'] = self::sanitize_color( $input['accent_color'] );
		}

		// Countries.
		if ( isset( $input['default_country'] ) ) {
			$clean['default_country'] = self::sanitize_country( $input['default_country'] );
		}
		if ( isset( $input['restrict_country'] ) ) {
			$clean['restrict_country'] = self::sanitize_country( $input['restrict_country'] );
		}
	...
	private static function input_text( $key, $type = 'text', $css_class = 'regular-text' ) {
	...
	private static function input_country_select( $key, $blank_label = '' ) {
		...
		printf(
			'<select name="estitofo[%s]" id="estitofo_%s" class="wc-est-country-select" style="min-width:300px;">',
			esc_attr( $key ),
			esc_attr( $key )
		);
