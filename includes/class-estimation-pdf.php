...
class Estitofo_PDF {
	...
	public function generate( $products, $total, $user_info = array() ) {
		...
		$this->brand    = self::hex_to_rgb( $primary_hex ? $primary_hex : '#2563eb' );
		$this->accent   = self::hex_to_rgb( $accent_hex ? $accent_hex : '#1e40af' );
		...
		try {
			...
			// Encryption unavailable — continue without PDF protection.
			unset( $e );
		}
		...
		$hide_logo = ( 'minimal' === $this->template );
		...
		if ( ! $hide_logo ) {
			...
			if ( ! $logo_drawn ) {
				...
				if ( $path && file_exists( $path ) ) {
                    ...
                    // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Logging is gated on WP_DEBUG.
					error_log(
						'[wc-estimation] PDF logo: resolve_logo_path returned nothing. URL=' . $logo_url
	...
	private function draw_locations_strip( $pdf, $locations, $company_name ) {
		...
		$cell_width = 180 / max( 1, count( $locations ) );
		$pdf->SetX( 15 );
		foreach ( $locations as $loc ) {
			$pdf->Cell( $cell_width, 4.5, $loc['name'], 0, 0, 'C' );
		}
		$pdf->Ln( 4.5 );
		$pdf->SetX( 15 );
		foreach ( $locations as $loc ) {
			$pdf->Cell( $cell_width, 4.5, $loc['phone'], 0, 0, 'C' );
		}
		$pdf->SetTextColor( 0, 0, 0 );
	}
	...
	private static function init_tcpdf_props( $pdf ) {
		...
		foreach ( $defaults as $name => $default ) {
			...
			try {
				...
				// Skip individual property failures — best-effort initialisation.
				unset( $e );
			}
		}
	}
	...
	public static function load_logo_data( $logo_url ) {
		...
		$bytes = @file_get_contents( $safe ); // phpcs:ignore WordPress.PHP.NoSilencedErrors, WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- reading a local file path, not a URL; wp_remote_get() is not applicable here.
		if ( ! is_string( $bytes ) || '' === $bytes ) {
			return false;
		}
		$safe_ext = strtoupper( pathinfo( $safe, PATHINFO_EXTENSION ) );
		$type     = ( 'JPEG' === $safe_ext ) ? 'JPG' : $safe_ext;
		return array(
			'data' => $bytes,
			'type' => $type,
			'path' => $safe,
	...
	public static function logo_diagnostic() {
		...
		if ( '' !== $url && 0 === $id && function_exists( 'attachment_url_to_postid' ) ) {
		...
		$detail['resolved_path'] = $path ? $path : '(unresolved)';
		if ( ! $path ) {
			return array(
				'ok'      => false,
		...
		$detail['safe_path'] = $safe ? $safe : '(none)';
		if ( ! $safe || ! file_exists( $safe ) ) {
			// The only way this branch runs in practice: PNG-with-alpha + no GD + no Imagick.
			$has_alpha = ( 'PNG' === $detail['ext'] && self::png_has_alpha( $path ) );
			if ( $has_alpha ) {
				return array(
					'ok'      => false,
					'message' => __(
	...
	private static function png_has_alpha( $path ) {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents, WordPress.PHP.NoSilencedErrors.Discouraged -- Need byte-accurate partial read; WP_Filesystem has no length-bounded read.
		$header = @file_get_contents( $path, false, null, 0, 26 );
		if ( false === $header || strlen( $header ) < 26 || 'PNG' !== substr( $header, 1, 3 ) ) {
			return false;
		}
		$color_type = ord( $header[25] );
		return ( 4 === $color_type || 6 === $color_type );
	}
	...
	private function url_to_path( $url ) {
		...
		if ( ( preg_match( '#^[A-Za-z]:[\\\\/]#', $url ) || 0 === strpos( $url, '/' ) ) && file_exists( $url ) ) {
			return $url;
		}
